...
export GATEWAY_URL=$(oc get route frontend -o jsonpath='{.spec.host}' -n $USERID-istio-system)
...
echo "GATEWAY_URL=$(oc get route frontend -o jsonpath='{.spec.host}' -n $USERID-istio-system)"
